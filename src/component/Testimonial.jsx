...
const TestimonialItem = ({ imageSrc, altText, content, clientName, profession }) => {
  ...
  return (
    <div className={`testimonial-item rounded p-4 p-lg-5 mb-5 d-flex flex-column justify-content-center justify-content-lg-start bg-white ${darkMode && 'bg-secondary'}`}>
      {imageSrc && <img className="testemonail__image mb-4" src={imageSrc} alt={altText} />}
      {content && <p style={{ fontSize: '80%' }} className="mb-4 text-lg-start text-primary fw-light">{content}</p>}
      {clientName && <h5 className="text-lg-start text-dark">{clientName}</h5>}
      {profession && <span className="text-primary text-lg-start text-purple">{profession}</span>}
    </div>
  )
};
